...
Use VS Code's ​Dev Container extension to build a containerized development environment. Leverage the khulnasoft/gptdb:latest image as the development environment to avoid repeated dependency installations and improve development efficiency.  
...
```bash
cp configs/gptdb-app-config.example.toml .devcontainer/dev.toml
```

- Start the service

```bash
gptdb start webserver --config .devcontainer/dev.toml
```
