...
GPT-DB supports the installation and use of various open-source and closed-source models. Different models have different requirements for environment and resources. If local model deployment is required, GPU resources are necessary. The API proxy model requires relatively few resources and can be deployed and started on a CPU machine.
...
Starting from version 0.7.0, GPT-DB uses uv for environment and package management, providing faster and more stable dependency management.
...
To run GPT-DB with the local model. You can modify the `configs/gptdb-local-vllm.toml` configuration file to specify the model path and other parameters.
...
```bash
uv run gptdb start webserver --config configs/gptdb-local-vllm.toml
```
...
To run GPT-DB with the local model. You can modify the `configs/gptdb-local-llama-cpp.toml` configuration file to specify the model path and other parameters.
...
```bash
uv run gptdb start webserver --config configs/gptdb-local-llama-cpp.toml
```
...
To run GPT-DB with Ollama proxy, you must provide the Ollama API base in the `configs/gptdb-proxy-ollama.toml` configuration file.
...
```bash
uv run gptdb start webserver --config configs/gptdb-proxy-ollama.toml
```
In the above command, `--config` specifies the configuration file, and `configs/gptdb-proxy-ollama.toml` is the configuration file for the Ollama proxy model, you can also use other configuration files or create your own configuration file according to your needs.

Optionally, you can also use the following command to start the webserver:
```bash
uv run python packages/gptdb-app/src/gptdb_app/gptdb_server.py --config configs/gptdb-proxy-ollama.toml
```
