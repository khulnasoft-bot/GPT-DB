...
First, you need to install the `gptdb graph_rag` library.
...
Set variables below in `configs/gptdb-graphrag.toml` file, let GPT-DB know how to connect to TuGraph.
...
```bash
uv run python packages/gptdb-app/src/gptdb_app/gptdb_server.py --config configs/gptdb-graphrag.toml
```
