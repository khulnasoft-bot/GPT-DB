...
title: GPT-DB Docker Build Guide
sidebar_label: Docker Build Guide
description: Comprehensive guide for building GPT-DB Docker images with various configurations
keywords:
  - GPT-DB
  - Docker
  - Build
...
# GPT-DB Docker Build Guide

This guide provides comprehensive instructions for building GPT-DB Docker images with various configurations using the `docker/base/build_image.sh` script.

## Overview

The GPT-DB build script allows you to create Docker images tailored to your specific requirements. You can choose from predefined installation modes or customize the build with specific extras, environment variables, and other settings.
...
GPT-DB requires Python 3.10 or higher. The default is Python 3.11, but you can specify a different version:
...
```bash
bash docker/base/build_image.sh --image-name mycompany/gptdb
```
...
This will generate `khulnasoft/gptdb-v1.0` for the default mode or `khulnasoft/gptdb-MODE-v1.0` for specific modes.
...
You can run `uv run install_help.py list` in your local GPT-DB repository to see all available extras.

### Environment Variables

GPT-DB build supports environment variables for specialized builds. The main environment variable used is `CMAKE_ARGS` which is particularly important for Llama-cpp compilation.
...
### Enterprise GPT-DB with PostgreSQL and Elasticsearch
...
| `--image-name` | Docker image name | `khulnasoft/gptdb` |
...
- [GPT-DB Documentation](https://github.com/khulnasoft/GPT-DB)
