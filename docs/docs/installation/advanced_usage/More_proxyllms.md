...
GPT-DB can be deployed on servers with lower hardware requirements through proxy LLMs. GPT-DB supports many proxy LLMs, such as OpenAI, Azure, DeepSeek, Ollama, and more.

## Installation and Configuration

Installing GPT-DB with proxy LLM support requires using the `uv` package manager for a faster and more stable dependency management experience.
...
Edit the `configs/gptdb-proxy-openai.toml` configuration file to specify your OpenAI API key:
...
Edit the `configs/gptdb-proxy-azure.toml` configuration file to specify your Azure OpenAI settings:
...
```bash
uv run gptdb start webserver --config configs/gptdb-proxy-azure.toml
```
...
Edit the `configs/gptdb-proxy-deepseek.toml` configuration file to specify your DeepSeek API key:
...
Edit the `configs/gptdb-proxy-ollama.toml` configuration file to specify your Ollama API base:
...
Create or edit a configuration file (e.g., `configs/gptdb-proxy-tongyi.toml`):
...
```bash
uv run gptdb start webserver --config configs/gptdb-proxy-tongyi.toml
```
...
Create or edit a configuration file (e.g., `configs/gptdb-proxy-zhipu.toml`):
...
```bash
uv run gptdb start webserver --config configs/gptdb-proxy-zhipu.toml
```
...
Create or edit a configuration file (e.g., `configs/gptdb-proxy-wenxin.toml`):
...
```bash
uv run gptdb start webserver --config configs/gptdb-proxy-wenxin.toml
```
