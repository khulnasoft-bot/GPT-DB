...
```bash
pip install "gptdb[agent,simple_framework]>=0.7.0" "gptdb_ext>=0.7.0" -U
pip install openai
```
...
from pydantic import BaseModel
from gptdb.agent import Agent, Action, AgentMessage, ActionOutput, AgentResource, ConversableAgent
