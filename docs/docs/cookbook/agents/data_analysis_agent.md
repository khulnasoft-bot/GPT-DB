...
本实践案例旨在帮助入门者使用 GPT-DB 创建数据分析的多智能体应用对 Superstore 数据集进行分析。通过本案例，您将学会如何部署项目、配置环境、准备数据，并成功运行针对 Superstore 销售数据的分析应用。

## 1. 项目部署

### 1.1 克隆代码库

首先，从 GitHub 克隆 GPT-DB 项目代码：
...
GPT-DB 支持多种部署方式，推荐使用 uv 工具进行依赖管理：
...
创建或编辑配置文件 `configs/gptdb-proxy-openai.toml`：
...
创建或编辑配置文件 `configs/gptdb-local-glm.toml`：
