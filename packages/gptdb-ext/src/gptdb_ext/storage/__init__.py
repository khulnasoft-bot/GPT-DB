...
def _import_pgvector() -> Tuple[Type, Type]:
    from gptdb_ext.storage.vector_store.pgvector_store import (
        PGVectorConfig,
        PGVectorStore,
    )
...
def _import_milvus() -> Tuple[Type, Type]:
    from gptdb_ext.storage.vector_store.milvus_store import (
        MilvusStore,
        MilvusVectorConfig,
    )
...
def _import_chroma() -> Tuple[Type, Type]:
    from gptdb_ext.storage.vector_store.chroma_store import (
        ChromaStore,
        ChromaVectorConfig,
    )
...
def _import_weaviate() -> Tuple[Type, Type]:
    from gptdb_ext.storage.vector_store.weaviate_store import (
        WeaviateStore,
...
def _import_oceanbase() -> Tuple[Type, Type]:
    from gptdb_ext.storage.vector_store.oceanbase_store import (
        OceanBaseConfig,
        OceanBaseStore,
    )
...
def _import_elastic() -> Tuple[Type, Type]:
    from gptdb_ext.storage.vector_store.elastic_store import (
...
def _import_openspg() -> Tuple[Type, Type]:
    from gptdb_ext.storage.knowledge_graph.open_spg import OpenSPG, OpenSPGConfig

    return OpenSPG, OpenSPGConfig


def _import_full_text() -> Tuple[Type, Type]:
    from gptdb_ext.storage.full_text.elasticsearch import ElasticDocumentStore
...
def scan_storage_configs():
    ...
    from gptdb.storage.base import IndexStoreConfig
    ...
    if _HAS_SCAN:
        return
    modules = [
        "gptdb_ext.storage.vector_store",
        "gptdb_ext.storage.knowledge_graph",
        "gptdb_ext.storage.graph_store",
    ]
