...
from gptdb.core.interface.media import MediaProcessor
...
from gptdb.model.base import ModelType, SupportedModel
...
class EmbeddingModelAdapter(ABC):
    """New Adapter for GPT-DB Embedding models"""
...
class LLMModelAdapter(ABC):
    ...
    def _set_conv_converted_messages(
        ...
        if system_messages:
            if len(system_messages) > 1:
                # Compatible with gptdb complex scenarios, the last system will protect
