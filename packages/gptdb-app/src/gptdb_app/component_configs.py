...
from gptdb.configs.model_config import MODEL_DISK_CACHE_DIR, resolve_root_path
from gptdb.util.executor_utils import DefaultExecutorFactory
from gptdb_app.config import ApplicationConfig, ServiceWebParameters
...
def initialize_components(
    ...
    from gptdb_app.initialization.embedding_component import (
        _initialize_embedding_model,
        _initialize_rerank_model,
    )
    from gptdb_app.initialization.scheduler import DefaultScheduler
    from gptdb_app.initialization.serve_initialization import register_serve_apps
    ...
    from gptdb_serve.agent.hub.controller import module_plugin
...
def _initialize_resource_manager(system_app: SystemApp):
    from gptdb.agent.expand.actions.react_action import Terminate
    ...
    from gptdb_serve.agent.resource.app import GptAppResource
    from gptdb_serve.agent.resource.datasource import DatasourceResource
    from gptdb_serve.agent.resource.knowledge import KnowledgeSpaceRetrieverResource
    from gptdb_serve.agent.resource.mcp import MCPSSEToolPack
    from gptdb_serve.agent.resource.plugin import PluginToolPack
...
def _initialize_operators():
    ...
    modules = ["gptdb_app.operators", "gptdb_serve.agent.resource"]
