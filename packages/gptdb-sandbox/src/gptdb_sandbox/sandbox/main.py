"""
GPT-DB Sandbox Main Entry Point
"""
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="GPT-DB Sandbox Server")
...
def run_sandbox_server(
    ...
    logger.info(f"Starting GPT-DB Sandbox server on {host}:{port}")

    try:
        initialize_sandbox(host=host, port=port, log_level=log_level)
    except KeyboardInterrupt:
        logger.info("Shutting down GPT-DB Sandbox server...")
