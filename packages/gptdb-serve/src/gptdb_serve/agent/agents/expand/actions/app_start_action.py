...
from gptdb_serve.agent.agents.expand.actions.intent_recognition_action import (
    IntentRecognitionInput,
)
from gptdb_serve.agent.db.gpts_app import GptsApp, GptsAppDao
        ...
        try:
            ...
            if not gpts_app:
                return ActionOutput(
                    is_exe_success=False,
                    content=ai_message,
                    view=f"[GPTDB Warning] Intent definition application cannot be "
            ...
            if TeamMode.NATIVE_APP.value == gpts_app.team_mode:
                return ActionOutput(
                    is_exe_success=False,
                    content=ai_message,
                    view="[GPTDB Warning] Native application connection startup is not"
            ...
            return ActionOutput(
                is_exe_success=False,
                content=ai_message,
                view=f"[GPTDB Warning] An exception occurred during the answering "
